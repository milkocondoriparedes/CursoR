...
#Ahora lo probamos
#1°prueba: x como list(1:4,matrix(1:4,1,4),matrix(1:4,4,1),matrix(1:4,2,2))
x.env$x<-list(1:4,matrix(1:4,1,4),matrix(1:4,4,1),matrix(1:4,2,2))
invMat(x.env$x)
x.env$x

#2° prueba: x como list(1:4,matrix(1:4,1,4),matrix(1:4,4,1),matrix(1:4,2,2)), silent=FALSE
x.env$x<-list(1:4,matrix(1:4,1,4),matrix(1:4,4,1),matrix(1:4,2,2))
invMat(x.env$x, silent = FALSE)
...
x.env$x<-list(diag(9),matrix(c(0.2,0.4,0.2,0.1,0.1,0.2),3,3),rbind(c(5,5,1,2),c(2,2,1,8),c(6,1,5,5),c(1,0,2,0)),matrix(1:6,2,3),cbind(c(3,5),c(6,5)),as.vector(diag(2)))
invMat(x.env$x,noninv = "matriz inadecuada")
x.env$x
